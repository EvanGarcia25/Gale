...
# Scraping Parameters (Edit these to change behavior)
START_YEAR = None  # None = earliest available year
END_YEAR = DEFAULT_END_YEAR  # Which year to end at
OUTPUT_FOLDER = DEFAULT_OUTPUT_FOLDER  # Where to save CSV files and hash index
...
def tables_to_csv(tables_list, folder_name, hash_index=None):
    """Convert a list of dictionaries (from grand_tables) to CSV files organized by year.
    
    Args:
        tables_list: List of dictionaries where each dict has {title: df_list}
        folder_name: Base folder path where year folders will be created
    ...
    for table_dict in tables_list:
        for title, df_list in table_dict.items():
            ...
            # Extract year from title (format: Yearbook{year}_Table...)
            year_match = re.search(r'Yearbook(\d{4})', title)
            year = year_match.group(1) if year_match else "Unknown"
            
            # Create year-specific folder
            year_folder = base_path / f"{year}Yearbook"
            ...
            # Remove "Yearbook{year}_" prefix from title for filename
            filename_base = re.sub(r'Yearbook\d{4}_', '', title)
            safe_filename = "".join(c for c in filename_base if c.isalnum() or c in (' ', '_', '-')).strip()
            
            # Windows has a 260 character path limit; truncate if needed
            if len(safe_filename) > MAX_FILENAME_LENGTH:
                safe_filename = safe_filename[:MAX_FILENAME_LENGTH]
            
            filepath = year_folder / f"{safe_filename}.csv"
    ...
    if skipped_count > 0:
        print(f"Saved {saved_count} table(s) to {base_path} (skipped {skipped_count} unchanged)")
    else:
        print(f"Saved {saved_count} table(s) to {base_path}")
...
def main():
    ...
    print("=" * 60)
    print(f"Start Year: {START_YEAR if START_YEAR else 'Earliest available'}")
    ...
    try:
        ...
        hash_index_path = os.path.join(OUTPUT_FOLDER, HASH_INDEX_FILENAME)
        ...
        hash_index = tables_to_csv(tables, OUTPUT_FOLDER, hash_index)
